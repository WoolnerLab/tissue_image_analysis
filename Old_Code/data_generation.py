...
c_files=sorted(glob.glob(input_dir+'20230203_1_*_us_8p6_*fr3*_*_conf.csv'))
#Path to directory where Matrices are stored
indata_dir="C:\\Users\\v35431nc\\Documents\\Lab_Stuff\\Multiple_people_Traces\\130522\\130522_20pc_stretch_1_fr1\\Matrices"
...
for f in c_files:
    ...
    A  = np.loadtxt(glob(indata_dir+'/Matrix_A_Tash_fr1_a_legs.txt')[0]) # Incidence matrix. Rows => edges; columns => vertices.
    B  = np.loadtxt(glob(indata_dir+'/Matrix_B_Tash_fr1_a_legs.txt')[0]) # Incidence matrix. Rows => cells; columns => edges. Values +/-1 for orientation
    C  = np.loadtxt(glob(indata_dir+'/Matrix_C_Tash_fr1_a_legs.txt')[0]) # Incidence matrix. Rows => cells; columns => vertices. 
    R  = np.loadtxt(glob(indata_dir+'/Matrix_R_Tash_fr1_a_legs.txt')[0]) # Coordinates of vertices
