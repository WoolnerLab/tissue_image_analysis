...
t,A,C,R,cell_centres,axisLength,savedir,edges_name,ExperimentFlag, NumberFlag, file_type):
    ...
    for j in range(0,N_e):
        ax.plot([beg_edge[j,0],end_edge[j,0]],[beg_edge[j,1],end_edge[j,1]],'k',alpha=1.0,linestyle ='-')
    if NumberFlag==1:
        for i in range(0,N_c):
            ax.plot(cell_centres[i,0],cell_centres[i,1],'k',marker ='o',markersize=2)
            ax.text(cell_centres[i][0], cell_centres[i][1], str(i),fontsize= 5,color='b')
    #Figure specs
    if ExperimentFlag == 1:
