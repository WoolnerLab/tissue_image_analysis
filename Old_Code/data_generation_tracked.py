...
input_dir='C:\\Users\\v35431nc\\Documents\\Lab_Stuff\\Movies_to_track\\unstretched\\20231019_1_IP_GFPCAAX-CheHis_uu_0p5_SumP\\5min_int/'
output_dir='C:\\Users\\v35431nc\\Documents\\Lab_Stuff\\Movies_to_track/Output/'
#########################
#User Input
#########################
c_files=sorted(glob(input_dir+'*_trace_conf.csv'))
#Path to directory where Matrices are stored
indata_dir="C:\\Users\\v35431nc\\Documents\\Lab_Stuff\\Movies_to_track\\unstretched\\20231019_1_IP_GFPCAAX-CheHis_uu_0p5_SumP\\5min_int\\Matrices"
...
for f in c_files:
    ...
    A  = np.loadtxt(glob(indata_dir+'/*Matrix_A*_fr'+frame+'*.txt')[0]) # Incidence matrix. Rows => edges; columns => vertices.
    B  = np.loadtxt(glob(indata_dir+'/*Matrix_B*_fr'+frame+'*.txt')[0]) # Incidence matrix. Rows => cells; columns => edges. Values +/-1 for orientation
    C  = np.loadtxt(glob(indata_dir+'/*Matrix_C*_fr'+frame+'*.txt')[0]) # Incidence matrix. Rows => cells; columns => vertices. 
    R  = np.loadtxt(glob(indata_dir+'/*Matrix_R*_fr'+frame+'*.txt')[0]) # Coordinates of vertices
    ...
    if stretch_type=='u' or t==0.0:
        ...
        pref_area=fileio.read_pref_area(output_dir, edges_name)


    fileio.write_parameters(data_dir,edges_name,stretch_type,t,pixel_size, micron_size,Gamma, Lambda, pref_area, area_scaling)
 ...
 ##############################
    ...
    R[:,1]=-R[:,1]
    cell_centres[:,1]=-cell_centres[:,1]
    major_stress_axis[:,1]=-major_stress_axis[:,1]
