...
output_dir='C:\\Users\\v35431nc\\Documents\\Lab_Stuff\\Parameter_Inference\Data/'


c_file=sorted(glob.glob(input_dir+'*_conf.csv'))[0]
f = open(c_file,'r')
lines = np.array(f.read().splitlines())[[25]]
f.close()
for l in lines:
    ...
    'major_shape_axis_alignment_rads', 'cell_zeta']
    cell_data_geom=np.transpose(np.vstack((cell_id, cell_perimeters, cell_areas,\
    shape_parameter, cell_circularity, cell_edge_count, major_shape_axis_alignment, zeta)))

    geom_df=pd.DataFrame(cell_data_geom, columns=geom_data_names)

    geom_df['time']=t_min
    geom_df['experiment']=edges_name.split('_')[0]+"_"+edges_name.split('_')[1]+"_"+edges_name.split('_')[2]
