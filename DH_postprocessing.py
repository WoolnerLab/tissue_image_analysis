...
script to read in data and create graphs. Note that the nearest neighbour shells are required for this to run, as well as the division flags

Outputs include the following text files for each cap:

bulk_pressure.txt - the bulk pressure from the centre of the the cap out
Counts_shells.txt  - number of cells dividing in each additional `shell' from the centre, divided by the number of cells within that shell.
Counts_shellsarea.txt - (cumulative number of cells dividing as a `shell' is added as we go from the centre out), divided by the cumulative number of cells included in the disc
Avg_alignment_per_shell.txt - mean cell alignment toward the centre (1 - radially aligned, 0 - tangentially aligned) per shell
Avg_alignment_per_shell_SEM.txt - standard error of the mean of cell alignment toward the centre
Avg_cell_area.txt - mean cell area per shell
Avg_cell_area_SEM.txt - standard error of the mean of the cell area per shell
Avg_cell_circ.txt - mean cell circularity per shell
Avg_cell_circ_SEM.txt - standard error of the mean of the cell circularity per shell
