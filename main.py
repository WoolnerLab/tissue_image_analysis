...
config_file=sorted(glob(input_dir+'*_conf.csv'))[0] #conf file name goes here
...
for l in lines: # for each trace
    
    #read in conf file
    edges_name,t_min, pixel_size, micron_size = fileio.read_conf_line(l)
    print(edges_name)
    exp_id=edges_name.split('_')[0]+'_'+edges_name.split('_')[1]+'_'+edges_name.split('_')[2] # extract experiment ID from trace file name
    t=t_min*60.0
    stretch_type=edges_name.split('_')[4][-1] # extract stretch type from trace file name
    frame=int(edges_name.split('_')[7][2:]) # extract frame from trace file name
    ...
    scale_pixels_to_microns = (micron_size/pixel_size)
    N_c=C.shape[0]
